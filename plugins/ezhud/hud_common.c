...
// "X" or "X Y" -> vec2f. "X" expands to (X, X). Used by widgets that take
// css-style shorthand padding cvars (frame_padding, frag_padding).
...
// Used by score_bar2 for team frag swatches.
...
void SCR_HUD_DrawScoresBar2(hud_t *hud)
{
	...
	for (i = 0; i < side_count; i++)
	{
		...
		box_w[i]  = max((int)strlen(score_str[i]), 3) * drawfuncs->StringWidth(font_px, 0, "0") + frag_pad.x * 2;
	}
