...
static void QCBUILTIN PF_getentity(pubprogfuncs_t *prinst, struct globalvars_s *pr_globals)
{
	...
	// Original fallback, probably dead end
	if (!es) {
...
// Thin wrapper so CSQC can read the engine's match-elapsed clock
// instead of duplicating the QTV print parser. Returns ms, -1 = unknown.
